...
Innerhalb des ES5 Standards gab es keine Möglichkeit Funktionen und Variablen strukturiert in Namensräumen zu organisieren oder dynamisch Code zu laden.
...
Typescirpt Decorator ermöglichen eine Implementierung des Dekorierer Pattern um Klassen,
Funktionen und Variablen deklarativ um Funktionalität zu erweitern. Dies ermöglicht eine zentrale Implementierung von
Funktionalität, welche mithilfe des @ Symbols und einer nachfolgenden Funktion an Elemente einer Anwendung angebracht werden kann.
Eine sehr vereinfachte Implementierung eines Decorators ist in Listing \ref{decorator-example} dargestellt.

\vspace{0.3cm}
\lstinputlisting[language=Java,label=decorator-example,caption=Decorator Beispiel]{kapitel2/decorator-example.ts}
\vspace{0.3cm}
