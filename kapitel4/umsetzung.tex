...
Zum Zeitpunkt der Login View Instanziierung wird der Localstorage im UserService über die Funktion \emph{getLocalMe} ausgelesen.
...
fängt der \emph{HTTP-Interceptor} den Fehler und leitet zur Login View zurück, um den Nutzer für eine erneute Authentifizierung aufzufordern.
...
Diese Komponente findet derzeit Verwendung in der Metrik, Graphen und Traces Sektion. Wird eine Applikation bzw. ein Zeitinterval über den Picker definiert,
...
so propagiert der Meta Picker ein Update an die äußere Eltern-Komponente. Die Variable \emph{\$event} beinhaltet dabei die vom Nutzer neu gesetzten Parameter.
...
In Kapitel \ref{Login-und-Authentifikation} Login und Authentifizierung wurde die lokale persistierung der Nutzerdaten im Localstorage
...
Da jedoch Funktionalität des \emph{UserServices} und des \emph{MetaPickers} auf dem Decorator aufbaut,
gilt es dieses Problem zu lösen.

Zuvor wurde in Kapitel emph{\ref{sec:services} Services, Providers und Dependency Injection erläutert, wie
